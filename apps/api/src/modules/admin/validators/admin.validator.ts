...
@Injectable()
export class AdminModuleValidator {
  ensureReady(summary: ModuleSummary): void {
    if (summary.status !== "READY_FOR_INTEGRATION") {
      throw new Error("Admin module is not ready for integration");
    }

    if (summary.capabilities.length === 0) {
      throw new Error("Admin module must expose at least one capability");
    }
  }
