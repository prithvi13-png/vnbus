...
export const ImageUpload = React.forwardRef<HTMLInputElement, ImageUploadProps>(
  ({ className, label = "Upload image", previewUrl, ...props }, ref) => (
    <label
      className={cn(
        "group grid aspect-video cursor-pointer place-items-center overflow-hidden rounded-lg border border-dashed border-gray-300 bg-white text-center transition-colors hover:border-blue-300 dark:border-gray-700 dark:bg-gray-950",
        className,
      )}
    >
      {previewUrl ? (
        ...
        <span className="grid gap-2 text-sm font-medium text-gray-700 dark:text-gray-200">
          <ImagePlus
            className="mx-auto h-6 w-6 text-blue-700 dark:text-blue-300"
            aria-hidden="true"
          />
          {label}
        </span>
