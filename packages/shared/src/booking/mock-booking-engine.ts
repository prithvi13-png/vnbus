...
const TERMS = [
  ...
  "Live tracking is marked Coming Soon for these bookings.",
];
...
export function createTicketRecord(booking: BookingRecord, now = new Date()): TicketRecord {
  ...
  const qrCode = createMockQrCode({
    ...
    verificationUrl: `https://verify.vriddhinexus.local/verify/${booking.bookingId}`,
  });
...
export function createMockTicketPdf(booking: BookingRecord): TicketPdfResponse {
  ...
  const lines = [
    ...
    "",
    "FOOTER",
    "This ticket is generated by the internal Vriddhi Nexus ticket model.",
  ];
